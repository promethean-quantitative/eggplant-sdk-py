...
- **Python 3.10+** and this package: `pip install eggplant-sdk` (or, to run the
  bundled example, clone this repo and `pip install -e .`).
...
- **Approvals in place.** The pUSD collateral adapter must be an ERC-1155
  ...
  - Safe wallets: `await approval.ensure_approvals(signer, relayer, rpc_url)`
    (see the `approvals_bootstrap.py` example) — it now covers the collateral
    adapter.
...
| `RPC_URL`                  | execute          | a Polygon RPC endpoint (defaults to a public one) |
...
```sh
WALLET=0xYourFunder python examples/sweep.py
```
...
python examples/sweep.py
```

Narrow to one event, or skip dust:

```sh
WALLET=0xYourFunder SLUG=some-event-slug MIN_SHARES=1 python examples/sweep.py
```

## 4. Using it from your own code

Two entry points, mirroring the rest of the package — a read-only planner and an
executor:
...
async def main():
    ...
    for r in await plan_sweep(data, gamma, wallet, opts):
        if r.classification.actionable():
            print(f"[{r.slug}] {r.title} — merge {r.classification.merge_pairs} "
                  f"/ convert {r.classification.convert_legs}")
    ...
    print(f"{summary.executed} settled, {summary.failed} failed")
...
The pure classification layer (`sweep.leg_sizes`, `sweep.classify_event`) needs
no network if you bring your own positions and legs.
